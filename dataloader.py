...
class GolfDB(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            event_num = np.random.randint(9) + 1
            if event_num <= 8:
                start_frame = events[event_num] - (self.seq_length-1) if events[event_num] - (self.seq_length-1) >= 0 else 0
            else:
                start_frame = np.random.randint(events[-1] + 1)
        ...
        if self.train:
            firstlast_img = [images[0], images[-1]]
            last_label = [labels[-1]]
            sample = {'images':np.asarray(firstlast_img), 'labels':np.asarray(last_label)}
        else:
...
if __name__ == '__main__':
    ...
    dataset = GolfDB(data_file='data/train_split_1.pkl',
                     vid_dir='data/videos160/',
                     seq_length=8,
                     transform=transforms.Compose([ToTensor(), norm]),
                     train=True)

    data_loader = DataLoader(dataset, batch_size=20, shuffle=False, num_workers=6, drop_last=False)
