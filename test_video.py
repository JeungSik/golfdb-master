...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--path', help='Path to video that you want to test', default='VID_20210707_095302.mp4')
    parser.add_argument('-s', '--seq-length', type=int, help='Number of frames to use per forward pass', default=4)
    ...
    # create mobile python script
    img_exam = torch.zeros(1, seq_length, 3, 160, 160)
    #  model.model[-1].export = False
    traced_script_module = torch.jit.trace(model, img_exam)
    from torch.utils.mobile_optimizer import optimize_for_mobile
    traced_script_module = optimize_for_mobile(traced_script_module)
    traced_script_module.save('golfdb.torchscript.pt')
    ...
    print('End Testing...... (소요시간 : {}초)'.format(end - start))

    for i, e in enumerate(events):
        cap.set(cv2.CAP_PROP_POS_FRAMES, e + (seq_length-1))
        _, img = cap.read()
